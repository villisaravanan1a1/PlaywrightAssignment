...
test.describe(`Edit incident flow `,async()=>{
    ...
    const homePageUrl = "https://dev192211.service-now.com/now/nav/ui/classic/params/target/ui_page.do%3Fsys_id%3D7201801147823110023cd879316d43e5"
    ...
    test(`TC001 - Create new incident which is a pre-requisite for edit`,async({page})=>{
        ...
        expect(callerIdPage.url()).toContain("https://dev192211.service-now.com/sys_user_list.do")
    ...
    test(`TCOO2 - verify you are able to delete the short description of the incident`,async({page})=>{
        console.log(`editing the incident ${incidentToEdit}`)
        ...
        await expect(mainContentFrame.locator("textarea[id='incident.description']")).toContainText("Updating the description of the incident")
        await page.pause()
        await page.frameLocator('iframe[name="gsft_main"]').locator('#check_container_incident_9d385017c611228701d22104cc95c371').click();
